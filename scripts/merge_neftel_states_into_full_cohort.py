...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--full",
        type=Path,
        default=Path("data/import_20260820/TP53 Dataset(preprocessed)/processed/full_cohort.h5ad"),
    )
    ...
    if state_ids != full_ids:
        raise SystemExit(
            f"Exact cell-ID join failed: old={len(state_ids)} new={len(full_ids)} overlap={len(state_ids & full_ids)}"
        )
    ...
    if "CrossSection" in states.obs:
        ...
        full.obs["CrossSection"] = [
            batch_map.get(str(cell), "not_available") for cell in full.obs_names
        ]
        full.obs["batch_source"] = [
            "data/neftel_qc-002.h5ad exact cell-ID join"
            if str(cell) in batch_map
            else "not_available"
            for cell in full.obs_names
        ]
    full.obs["state_source"] = [
        "data/neftel_qc-002.h5ad exact cell-ID join" if str(cell) in state_map else "not_available"
        for cell in full.obs_names
    ]
    ...
    result = {
        ...
        "input_full": {
            "path": str(args.full),
            "sha256": sha256(args.full),
            "shape": list(full.shape),
        },
        "input_state_file": {
            "path": str(args.states),
            "sha256": sha256(args.states),
            "cells_joined": len(state_ids),
        },
        "output": {
        ...
        "state_counts": {
            str(k): int(v) for k, v in full.obs["derived_state"].value_counts().items()
        },
