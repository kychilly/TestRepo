...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        evidence = read_jsonl(args.evidence) if args.evidence else []
        ...
        if args.join_output:
            write_jsonl(args.join_output, [item.to_dict() for item in inputs])
        write_jsonl(args.output, [item.to_validator_payload() for item in inputs])
    ...
    print(
        json.dumps(
            {
                "status": "completed",
                "inputs_written": len(inputs),
                "output": str(args.output),
                "join_output": str(args.join_output) if args.join_output else None,
                "payload_shape": "simplified_validator_payload",
            },
            sort_keys=True,
        )
    )
