...
def main(argv: list[str] | None = None) -> int:
    ...
    common = [
        gene for gene in neftel.var_names.astype(str) if gene in set(cgga.var_names.astype(str))
    ]
    ...
    result = {
        ...
        "inputs": {
            "neftel": {"path": str(args.neftel), "sha256": sha256(args.neftel)},
            "cgga": {"path": str(args.cgga), "sha256": sha256(args.cgga)},
        },
        "common_genes": len(common),
        "cohort_counts": {str(k): int(v) for k, v in combined.obs["Cohort"].value_counts().items()},
        "idh_counts": {
            str(k): int(v) for k, v in combined.obs["IDH_status"].astype(str).value_counts().items()
        },
        "finite_values": bool(np.isfinite(combined.X).all()),
        "cell_state_external_truth": False,
        "next_actions": [
            "Use this file for patient-level CGGA IDH evaluation; do not score CGGA cell states."
        ],
    }
