...
def toy_data() -> CellData:
    ...
    return CellData(
        X, patients, np.array([f"c{i}" for i in range(24)]), states, ("g1", "g2", "g3", "g4")
    )
...
def splits_file(tmp_path: Path) -> Path:
    ...
    path.write_text(
        json.dumps({"train": ["p1", "p2"], "validation": ["p3"], "test": ["p4"]}), encoding="utf-8"
    )
    return path
...
def test_overlap_rejection(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps({"train": ["p1"], "validation": ["p1"], "test": ["p2"]}), encoding="utf-8"
    )
...
def test_harmony_non_applicability_is_structured() -> None:
    with pytest.raises(Exception, match="unseen"):
...
def test_common_evaluation_outputs_cell_and_patient_rows(tmp_path: Path) -> None:
    ...
    assert {"probability_AC", "probability_MES", "probability_NPC", "probability_OPC"} <= cells[
        0
    ].keys()
